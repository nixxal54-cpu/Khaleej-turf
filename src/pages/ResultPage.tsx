      ...
      <main className="max-w-lg mx-auto px-4 mt-6">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="p-6 bg-white border border-slate-200 rounded-3xl shadow-sm relative overflow-hidden">
          <div className="text-center mb-6">
            ...
            <p className="text-slate-500 font-medium mt-1">{safeFormatDate(event.date, 'MMM do, yyyy')} • {event.startTime}–{event.endTime}</p>
          ...
          <div className="space-y-6">
            <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100">
              ...
              <ul className="space-y-3">
                {Array.isArray(result.teamA) ? result.teamA.map((p,i) => <li key={p.id} className="font-medium text-slate-700 flex gap-3 text-lg"><span className="text-slate-400 w-5">{i+1}.</span> {p.name}</li>) : <li className="text-red-500">Invalid team data</li>}
              </ul>
            </div>
            ...
            <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100">
              ...
              <ul className="space-y-3">
                {Array.isArray(result.teamB) ? result.teamB.map((p,i) => <li key={p.id} className="font-medium text-slate-700 flex gap-3 text-lg"><span className="text-slate-400 w-5">{i+1}.</span> {p.name}</li>) : <li className="text-red-500">Invalid team data</li>}
              </ul>
            </div>
          </div>
